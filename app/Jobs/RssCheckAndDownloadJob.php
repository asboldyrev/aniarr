...
class RssCheckAndDownloadJob implements ShouldQueue
{
    ...
    public function handle(SonarrClient $sonarrClient, QBittorrentClient $qBittorrentClient, RssParser $rssParser): void
    {
        ...
        $seriesList = Series::whereNotNull('rss_url')
        ...
        foreach ($seriesList as $series) {
            try {
                ...
                $parseItems = $rssParser->parseFeed($series->rss_url);
                if (empty($parseItems->items)) {
                    $logger->warning('Проверка пропущена: отсутствует данные в RSS-ленте');

                    continue;
                }

                $picked = app(BestTorrentPicker::class)->pick($parseItems);
                if ($picked === null) {
                    $logger->warning('Проверка пропущена: не найден подходящий torrent-файл');
                ...
                $logger->exception($e, $series->rss_url);
            }
        }
    }
